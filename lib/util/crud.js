...
module.exports = {
  ...
  remove: function (watcher, id, esClient) {
    ...
    esClient.delete({
      ...
      if (error) {
        debugError('Document %s to be deleted but not found', id);
      } else {
  ...
  update: function (watcher, id, partialDocument, esClient) {
    ...
    esClient.update({
      ...
      type: watcher.type,
      id: id.toString(),
      body: { doc: parser.object(partialDocument) }
    }, function (error) {
      if (!error) {
        debugElasticsearch('Updatee %s in %s/%s (index/type)', id, watcher.index, watcher.type);
      } else {
        debugElasticsearch('An error occured during Update :');
        debugElasticsearch(error);
      }
    });
  }
