...
var tail = function () {
  ...
  const oplog = MongoOplog(process.env.MONGO_OPLOG_URL);
  ...
  oplog.on('update', doc => {
    ...
    if (watcher) {
      ...
      } else if(doc.o2 && doc.o2._id && doc.o && doc.o.$set) {
        
        // For partial update (work only with $set for unknown)
        Crud.update(watcher, doc.o2._id, doc.o.$set, EsClient);
        
      } else {
        
        debugError('Update Operation is not handled yet')
        debugError(doc)
